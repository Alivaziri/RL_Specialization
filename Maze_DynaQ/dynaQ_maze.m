function [Q,ets,numFinishes,Model_ns,Model_nr,cr] = dynaQ_maze(alpha,epsilon,gamma,nPlanningSteps,mz_fn,s_start,s_end,MAX_N_STEPS)
% DYNAQ_MAZE - Runs the dynaQ planning algorithm on a "maze" mz_fn
%   
% Input: 
...
%Q = mean( [-1,0] )*ones(nStates,nActions);
Q = -3*ones(nStates,nActions);
%Q = zeros(nStates,nActions);

% storage for the sequence of states we have observed
%  (these are stored as indices ... as produced by the matlab command "sub2ind.m")
seen_states = [];
% storage for the sequence of actions taken in each state 
...
%     r_ati = sample_discrete( pro_action, 1, 1 );
    ...
    %fprintf('Q(r_sti=%5d,r_ati=%5d) = %10.6f\n',r_sti,r_ati,Q(r_sti,r_ati)); 
